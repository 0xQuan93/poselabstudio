...
export function ExportTab({ mode = 'reactions' }: ExportTabProps) {
  ...
  useEffect(() => {
    // This effect is moved below handleExport
  }, []);
  ...
  const handleExport = () => {
    ...
    useUserStore.getState().recordGamifiedAction('export_daily').then(reward => {
      if (reward > 0) {
        useToastStore.getState().addToast(`+${reward} LP for your first export today!`, 'info');
      }
    });
  };
  ...
  }, [isAvatarReady, isExporting, exportFormat, resolution, includeLogo, transparentBg, handleExport]);
